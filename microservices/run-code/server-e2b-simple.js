...
app.post('/run_code', async (req, res) => {
  ...
  try {
    ...
    if (language === 'python') {
      ...
      // Check if code needs numpy/scipy and install if needed
      const needsNumpy = source.includes('numpy') || source.includes('np.');
      const needsScipy = source.includes('scipy') || source.includes('sp.');
      const needsSympy = source.includes('sympy') || source.includes('sp.');
      
      if (needsNumpy || needsScipy || needsSympy) {
        console.log('Installing required libraries...');
        let installCmd = 'pip install -q';
        if (needsNumpy) installCmd += ' numpy';
        if (needsScipy) installCmd += ' scipy';  
        if (needsSympy) installCmd += ' sympy';
...
// Start server
app.listen(PORT, () => {
  console.log(`E2B Code execution service (simple) running on port ${PORT}`);
